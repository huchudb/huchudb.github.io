...
function renderNaviStatsWidget(payload) {
  ...
  const groups = [
    { key: "re_collateral",    label: "부동산 담보대출" },
    { key: "personal_credit",  label: "개인 신용대출" },
    { key: "corporate_credit", label: "법인 신용대출" },
    { key: "stock",            label: "스탁론" },
    { key: "medical",          label: "의료사업자대출" },
    { key: "art",              label: "미술품 담보대출" },
    { key: "receivable",       label: "매출채권 유동화" },
    { key: "enote",            label: "전자어음" },
    { key: "auction_dividend", label: "경매배당금 담보대출" },
  ];

  const data = payload?.productGroups || {};

  // ✅ 아이콘 이미지(캔바 디자인 기준) — /assets/navi-icons/*.png
  const ICON_BASE = "/assets/navi-icons";
  const ICON_VER  = "20251231";
  const iconImg = (k) => {
    const src = `${ICON_BASE}/${encodeURIComponent(k)}.png?v=${ICON_VER}`;
    return `<img class="navi-tile__img" src="${src}" alt="" loading="lazy" decoding="async" />`;
  };
...
const tileHtml = groups.map((g, i) => {
    ...
    return `
      <div class="navi-tile" style="animation-delay:${delay}ms">
        <div class="navi-tile__iconimg" aria-hidden="true">${iconImg(g.key)}</div>
