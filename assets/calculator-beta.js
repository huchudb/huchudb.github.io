// assets/calculator-beta.js
// 베타 전용: 물건 종류별 LTV/금리 기준을 서버에서 불러와 사용하는 버전
...
async function fetchLoanConfig() {
  try {
...
function getLoanConfigForType(propertyType) {
  if (!LOAN_CONFIG || !LOAN_CONFIG.byType) {
    throw new Error('대출 기준 설정을 불러오지 못했습니다. 잠시 후 다시 시도해주세요.');
  }
  const cfg = LOAN_CONFIG.byType[propertyType];
  if (!cfg) {
    throw new Error(`"${propertyType}"에 대한 대출 기준 설정이 없습니다.`);
  }
  return cfg;
}
...
function updatePreview(inputId, viewId) {
  ...
  if (pv) pv.textContent = formatKoreanCurrency(Number(raw || '0'));
}
...
const monthlyPayment = (principal, annualRate) =>
  principal * annualRate / 12;

/**
 * region 은 현재 LTV/금리에는 직접 사용하지 않고,
 * 필요시 통계/로그용으로 남겨둔다.
 */
function computeLtv({
  region,
  ...
  const cfg = getLoanConfigForType(propertyType);
  const ratio = cfg.maxLtv;
    ...
    if (!this.el) {
      ...
      if (this.el) {
        this.el.addEventListener('click', (e) => {
          if (e.target.dataset.close === 'true') calcModal.close();
        });
      }
      if (this.closeBtn) {
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') calcModal.close();
      });
    }
...
function buildInputChips({
  ...
  if (deduction > 0) {
    chips.push(`<span class="chip"><span class="label">상환/반환</span><span class="val">${formatKoreanCurrency(deduction)}</span></span>`);
  }
  if (shareUsed) {
  ...
  return `
    <div class="kpis">
      <div class="kpi">
        ...
        <div class="kpi-value">${formatKoreanCurrency(val1)}</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">${label2}</div>
        <div class="kpi-value">${formatKoreanCurrency(val2)}</div>
      </div>
    </div>`;
}

const kpisForDenied = (req, maxReq) =>
  kpisHTML('요청 금액(총 실행)', req, '가능 요청금액', Math.max(0, maxReq));

function kpisForDeniedHTML(requestedLoan, maxReq) {
  return kpisForDenied(requestedLoan, maxReq);
}
...
async function serverGetCount() {
  ...
  return {
    ...
    byType: j.byType || { '아파트': 0, '다세대/연립': 0, '단독/다가구': 0, '토지/임야': 0 }
  };
}
...
function renderDaily({ total, byType }) {
  ...
  if (byType) {
    const set = (id, v) => {
      const el = typeCountEl(id);
      if (el) el.textContent = (v || 0).toLocaleString('ko-KR');
    };
    set('aptCount', byType['아파트']);
    set('multiCount', byType['다세대/연립']);
    set('houseCount', byType['단독/다가구']);
    set('landCount', byType['토지/임야']);
  }
}
...
async function bumpDailyUsersOncePerDay(propertyTypeKor) {
  try {
    const data = await serverIncrementOncePerDay(propertyTypeKor);
...
async function onCalculate() {
  ...
  if (missing.length) {
    ...
    `, { type: 'info', text: '안내' });
    const focusId = ids[missing[0]];
    if (focusId) {
      setTimeout(() => document.getElementById(focusId).focus({ preventScroll: true }), 150);
    }
    return;
  }
  ...
  if (deduction > requestedLoan) {
    alert('상환/반환 금액이 "필요한 대출 금액"을 초과할 수 없습니다. 자동조정합니다.');
    deduction = requestedLoan;
  }
  if (deduction > seniorLoan) {
    alert('상환/반환 금액이 선순위 잔액을 초과할 수 없습니다. 자동조정합니다.');
    deduction = seniorLoan;
  }
  ...
  try {
    r = computeLtv({
      region, propertyType, marketValue, seniorLoan,
      deduction, requestedLoan, sharePercent
    });
  } catch (e) {
    const inputs = buildInputChips({
      region, propertyType, marketValue,
      seniorLoan, requestedLoan, deduction, sharePercent
    });
    calcModal.open(`
      ${inputs}
      <div class="notice error"><p>${e.message}</p></div>
    `, { type: 'error', text: '오류' });
    return;
  }
  ...
  if (seniorLoan > r.limit || r.status === 'exceeded') {
    const kpis = kpisForDeniedHTML(requestedLoan, r.maxRequested);
    calcModal.open(`
      ${inputs}
      ${kpis}
  ...
  // 설정에서 가져온 금리 (없으면 기본값)
  const rateMin = r.rateMin ?? 0.068;
  const rateMax = r.rateMax ?? 0.148;

  const mMin = monthlyPayment(requestedLoan, rateMin);
  const mMax = monthlyPayment(requestedLoan, rateMax);
  const feeMin = requestedLoan * 0.01;
  const feeMax = requestedLoan * 0.015;

  const rateMinPct = (rateMin * 100).toFixed(1);
  const rateMaxPct = (rateMax * 100).toFixed(1);

  const kpis = `
    <div class="kpis">
      <div class="kpi">
        <div class="kpi-label">요청 금액(총 실행)</div>
        <div class="kpi-value">${formatKoreanCurrency(requestedLoan)}</div>
      </div>
      <div class="kpi">
        <div class="kpi-label">추가 가능 금액</div>
        <div class="kpi-value">${formatKoreanCurrency(addAvail)}</div>
      </div>
    </div>`;
  ...
  const breakdownCosts = `
    <div class="breakdown-card" aria-labelledby="costsTitle">
      ...
      <div class="card-section">
        ...
        <div class="chip-row rate">
          <span class="chip plain">
            <span class="label rate-min">${rateMinPct}%</span>
            <span class="val">${formatKoreanCurrency(mMin)}</span>
          </span>
          <span class="chip-tilde">~</span>
          <span class="chip plain">
            <span class="label rate-max">${rateMaxPct}%</span>
            <span class="val">${formatKoreanCurrency(mMax)}</span>
          </span>
        </div>
      </div>
      <div class="card-section compact">
        ...
        <div class="chip-row rate">
          <span class="chip plain">
            <span class="label rate-min">1%</span>
            <span class="val">${formatKoreanCurrency(feeMin)}</span>
          </span>
          <span class="chip-tilde">~</span>
          <span class="chip plain">
            <span class="label rate-max">1.5%</span>
            <span class="val">${formatKoreanCurrency(feeMax)}</span>
          </span>
        </div>
      </div>
    </div>`;
    ...
    <div class="breakdown-card" aria-labelledby="infoTitle">
      ...
      <div class="rowlist">
        <div class="row">
          ...
          <span class="label">대출기간</span>
          <span class="value">평균 1년</span>
        </div>
        <div class="row">
          <span class="icon-dot dot-green"></span>
          <span class="label">중도상환수수료</span>
          <span class="value">없음</span>
        </div>
        <div class="row">
          <span class="icon-dot dot-slate"></span>
          <span class="label">대출연장수수료</span>
          <span class="value">0%~1% (업체별 상이)</span>
        </div>
      </div>
    </div>`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (calcBtn) {
    ...
    calcBtn.textContent = '대출 기준 불러오는 중...';
  }
  ...
  if (calcBtn) {
    ...
    calcBtn.textContent = '대출 가능성 확인하기';
